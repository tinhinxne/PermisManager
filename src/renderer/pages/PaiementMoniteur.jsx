...
function genererRapportGroupePDF(fiches, dateDebut, dateFin) {
  ...
  const versementsFiltrés = fiches.flatMap(f =>
    f.versements
      .filter(v => {
  ...
  const totalRapport = versementsFiltrés.reduce((s, v) => s + Number(v.montant || 0), 0);
  ...
  const bW = (W - 28 - 8) / 3;
  [
    { label: "Versements",     val: String(versementsFiltrés.length) },
    { label: "Candidats",      val: String(new Set(versementsFiltrés.map(v => v.candidat.idCandidat)).size) },
  ...
  doc.setFont("helvetica", "bold"); doc.setFontSize(9); doc.setTextColor(43, 83, 126);
  doc.text(`DÉTAIL DES VERSEMENTS (${versementsFiltrés.length})`, 14, y); y += 5;
  doc.setFillColor(43, 83, 126); doc.rect(14, y, totW, 9, "F");
  doc.setFont("helvetica", "bold"); doc.setFontSize(8); doc.setTextColor(255, 255, 255);
  cols.forEach((c, i) => doc.text(c.label, xs[i] + 2, y + 6));
  y += 9;

  versementsFiltrés.forEach((v, ri) => {
    if (y + 10 > H - 20) { doc.addPage(); y = 20; }
    if (ri % 2 === 1) { doc.setFillColor(248, 250, 252); doc.rect(14, y, totW, 8, "F"); }
...
function ExportModal({ fiches, onClose }) {
  ...
  const toggleAll = () => {
    ...
    else           { setSelectedIds(new Set(fichesFiltrees.map(f => f.idCandidat))); setSelectAll(true); }
  };
  ...
  const versementsInPeriode = (f) => f.versements.filter(v => {
    const d = new Date(v.dateVersement);
    return (!dateDebut || d >= new Date(dateDebut)) && (!dateFin || d <= new Date(dateFin + "T23:59:59"));
  });
  const totalVersementsExport = fichesExport.reduce((s, f) => s + versementsInPeriode(f).reduce((ss, v) => ss + Number(v.montant || 0), 0), 0);
  const nbVersementsExport    = fichesExport.reduce((s, f) => s + versementsInPeriode(f).length, 0);

  const handlePreview = () => {
    if (fichesExport.length === 0 || nbVersementsExport === 0) return;
    if (format === "csv") { exporterCSV(fichesExport, dateDebut, dateFin); onClose(); return; }
  ...
  const handleTelecharger = () => {
    if (previewDoc) previewDoc.save(`Rapport_Paiements_${new Date().toISOString().slice(0, 10)}.pdf`);
    if (format === "both") exporterCSV(fichesExport, dateDebut, dateFin);
    onClose();
  };

  const inp = { padding: "9px 12px", border: "1.5px solid #e2e8f0", borderRadius: 9, fontSize: 13, outline: "none", background: "#f8fafc", color: "#1e293b", width: "100%", boxSizing: "border-box" };
  ...
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(15,23,42,0.65)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "#fff", borderRadius: 18, width: "100%", maxWidth: step === "preview" ? 820 : 580, maxHeight: "92vh", display: "flex", flexDirection: "column", boxShadow: "0 32px 80px rgba(0,0,0,0.25)", overflow: "hidden", transition: "max-width .2s ease" }}>
        <div style={{ background: "linear-gradient(135deg,#2b537e,#1e3a5f)", padding: "18px 22px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 800, color: "#fff" }}>{step === "form" ? "Export des paiements" : "Aperçu du rapport"}</div>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.65)", marginTop: 2 }}>{step === "form" ? "PDF · CSV · Filtrage par période et candidat" : "Vérifiez avant de télécharger"}</div>
          </div>
          ...
          <>
            <div style={{ overflowY: "auto", flex: 1, padding: "20px 22px", display: "flex", flexDirection: "column", gap: 18 }}>
              <div>
                ...
                {(dateDebut || dateFin) && (
                  <button onClick={() => { setDateDebut(""); setDateFin(""); }} style={{ marginTop: 6, background: "none", border: "none", fontSize: 11, color: "#94a3b8", cursor: "pointer", padding: 0 }}>✕ Effacer la période</button>
                )}
              </div>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {[["pdf", "📄 PDF"], ["csv", "📊 Excel/CSV"], ["both", "📄+📊 Les deux"]].map(([val, label]) => (
                    <button key={val} onClick={() => setFormat(val)} style={{ flex: 1, padding: "9px 6px", borderRadius: 9, border: `2px solid ${format === val ? "#2b537e" : "#e2e8f0"}`, background: format === val ? "#eff6ff" : "#f8fafc", color: format === val ? "#2b537e" : "#64748b", fontWeight: format === val ? 700 : 500, fontSize: 12, cursor: "pointer" }}>
                      {label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <input type="text" placeholder="🔍 Filtrer les candidats…" value={recherche} onChange={e => setRecherche(e.target.value)} style={{ ...inp, marginBottom: 8 }} />
                <div onClick={toggleAll} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderRadius: 8, background: "#f1f5f9", cursor: "pointer", marginBottom: 6, userSelect: "none" }}>
                ...
                <div style={{ maxHeight: 200, overflowY: "auto", border: "1px solid #e2e8f0", borderRadius: 10, background: "#f8fafc" }}>
                  {fichesFiltrees.map((f, i) => (
                    <div key={f.idCandidat} onClick={() => toggleCandidat(f.idCandidat)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", cursor: "pointer", borderBottom: i < fichesFiltrees.length - 1 ? "1px solid #e2e8f0" : "none", background: selectedIds.has(f.idCandidat) ? "#eff6ff" : "transparent", userSelect: "none" }}>
                      ...
                      <span style={{ padding: "2px 8px", borderRadius: 20, fontSize: 10, fontWeight: 700, background: f.statut.bg, color: f.statut.color }}>{f.statut.label}</span>
                    </div>
                  ))}
                  {fichesFiltrees.length === 0 && <div style={{ padding: 20, textAlign: "center", color: "#94a3b8", fontSize: 13 }}>Aucun candidat trouvé</div>}
                </div>
              </div>
            ...
            <div style={{ padding: "14px 22px", borderTop: "1px solid #e2e8f0", display: "flex", gap: 10, flexShrink: 0, background: "#f8fafc" }}>
              <button onClick={onClose} style={{ flex: 1, padding: 11, borderRadius: 10, border: "1px solid #e2e8f0", background: "#fff", color: "#64748b", cursor: "pointer", fontWeight: 600, fontSize: 13 }}>Annuler</button>
              <button onClick={handlePreview} disabled={selectedIds.size === 0 || nbVersementsExport === 0} style={{ flex: 2, padding: 11, borderRadius: 10, border: "none", background: selectedIds.size === 0 || nbVersementsExport === 0 ? "#94a3b8" : "#2b537e", color: "#fff", fontWeight: 700, fontSize: 13, cursor: selectedIds.size === 0 || nbVersementsExport === 0 ? "not-allowed" : "pointer" }}>
                {nbVersementsExport === 0 ? "Aucun versement dans la période" : format === "csv" ? `Exporter le CSV (${nbVersementsExport} versement${nbVersementsExport > 1 ? "s" : ""})` : `👁 Aperçu (${nbVersementsExport} versement${nbVersementsExport > 1 ? "s" : ""})`}
              </button>
            </div>
          </>
        )}
        ...
        {step === "preview" && (
          <>
            <div style={{ flex: 1, background: "#525659", padding: 12, overflow: "hidden" }}>
              {previewUrl && <iframe src={previewUrl} title="Aperçu du rapport" style={{ width: "100%", height: "100%", minHeight: 420, border: "none", borderRadius: 6, background: "#fff" }} />}
            </div>
            <div style={{ padding: "14px 22px", borderTop: "1px solid #e2e8f0", display: "flex", gap: 10, flexShrink: 0, background: "#f8fafc" }}>
              <button onClick={() => { setPreviewUrl(null); setPreviewDoc(null); setStep("form"); }} style={{ flex: 1, padding: 11, borderRadius: 10, border: "1px solid #e2e8f0", background: "#fff", color: "#64748b", cursor: "pointer", fontWeight: 600, fontSize: 13 }}>← Modifier</button>
              <button onClick={handleTelecharger} disabled={!previewDoc} style={{ flex: 2, padding: 11, borderRadius: 10, border: "none", background: previewDoc ? "#166534" : "#94a3b8", color: "#fff", fontWeight: 700, fontSize: 13, cursor: previewDoc ? "pointer" : "not-allowed" }}>
...
function HistoriqueModal({ candidat, onClose }) {
  ...
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(15,23,42,0.65)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "#fff", borderRadius: 18, width: "100%", maxWidth: 820, maxHeight: "92vh", display: "flex", flexDirection: "column", boxShadow: "0 32px 80px rgba(0,0,0,0.25)", overflow: "hidden" }}>
        <div style={{ background: "linear-gradient(135deg,#6d28d9,#5b21b6)", padding: "18px 22px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 800, color: "#fff" }}>Historique des paiements</div>
        ...
        <div style={{ flex: 1, background: "#525659", padding: 12, overflow: "hidden" }}>
          {previewUrl
            ? <iframe src={previewUrl} title="Aperçu historique" style={{ width: "100%", height: "100%", minHeight: 460, border: "none", borderRadius: 6, background: "#fff" }} />
            : <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", minHeight: 460, color: "#ccc", fontSize: 13 }}>⏳ Génération du PDF…</div>}
        </div>
        <div style={{ padding: "14px 22px", borderTop: "1px solid #e2e8f0", display: "flex", gap: 10, flexShrink: 0, background: "#f8fafc" }}>
          <button onClick={onClose} style={{ flex: 1, padding: 11, borderRadius: 10, border: "1px solid #e2e8f0", background: "#fff", color: "#64748b", cursor: "pointer", fontWeight: 600, fontSize: 13 }}>Fermer</button>
          <button onClick={() => { if (doc) { doc.save(`Historique_${candidat.nom}_${candidat.prenom}.pdf`); onClose(); } }} disabled={!doc} style={{ flex: 2, padding: 11, borderRadius: 10, border: "none", background: doc ? "#6d28d9" : "#94a3b8", color: "#fff", fontWeight: 700, fontSize: 13, cursor: doc ? "pointer" : "not-allowed" }}>
            ⬇ Télécharger le PDF
          </button>
        </div>
      </div>
...
// ─── Composant Principal ──────────────────────────────────────────────────────
const PaymentsMoniteur = () => {
  // ✅ Permissions via hook (lit currentUser.id automatiquement)
  const {
    CAN_VIEW_ALL_CANDIDATES: canViewAll,
    CAN_ADD_PAYMENT:         canAddPayment,
  } = useMyPermissions();

  // ✅ currentUser via hook Auth (même source que MesCandidats)
  ...
  const [seances,        setSeances]        = useState([]); // ✅ pour filtrer par moniteur
  ...
  const fetchData = useCallback(async () => {
    try {
      const [payments, candidates, rawSeances] = await Promise.all([
        window.electron.getPayments(),
        window.electron.getCandidats(),
        window.electron.getSeances(), // ✅ même appel que MesCandidats
      ]);
  ...
  // ── Candidats visibles — MÊME logique que MesCandidats ───────────────────────
  const candidatesVisibles = useMemo(() => {
    if (canViewAll) return allCandidates;

    // Trouver les candidats du moniteur connecté via ses séances (identique à MesCandidats)
    const moniteurId = currentUser?.id;
    const mesSeances = seances.filter(s => s.moniteur_id === moniteurId);
    ...
    return allCandidates.filter(c => candidatIdSet.has(c.idCandidat));
  }, [allCandidates, seances, canViewAll, currentUser?.id]);

  // ── Fiches candidats (versements rattachés) ──────────────────────────────────
  ...
  // ── Stats ────────────────────────────────────────────────────────────────────
  const totalEncaisse = useMemo(() => fichesCandidats.reduce((s, f) => s + f.totalVerse, 0), [fichesCandidats]);
  const totalRestant  = useMemo(() => fichesCandidats.reduce((s, f) => s + f.montantRestant, 0), [fichesCandidats]);
  const nbDebiteurs   = useMemo(() => fichesCandidats.filter((f) => f.montantRestant > 0).length, [fichesCandidats]);
  const revenuMois    = useMemo(() => {
    const mois  = new Date().getMonth();
    const annee = new Date().getFullYear();
    return fichesCandidats
      .flatMap((f) => f.versements)
    ...
    <div className="container">
      <div className="main">
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
          <input type="text" placeholder="🔍 Rechercher un candidat…" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} style={{ flex: 1, minWidth: 200, padding: "10px 14px", border: "1.5px solid #e2e8f0", borderRadius: 10, outline: "none", fontSize: 13, background: "#fff" }} />
          <div style={{ display: "flex", background: "#f1f5f9", borderRadius: 10, padding: 3, gap: 2 }}>
            {[["tous","Tous"],["en_cours","En cours"],["jamais","Jamais payé"],["solde","Soldés"]].map(([val, label]) => (
              <button key={val} onClick={() => setFiltreStatut(val)} style={{ padding: "7px 13px", borderRadius: 8, border: "none", fontWeight: 600, fontSize: 12, cursor: "pointer", background: filtreStatut === val ? "#fff" : "transparent", color: filtreStatut === val ? "#2b537e" : "#64748b", boxShadow: filtreStatut === val ? "0 1px 4px rgba(0,0,0,0.1)" : "none" }}>
                {label}
              </button>
            ))}
          </div>
          <button onClick={() => setShowExport(true)} style={{ padding: "10px 18px", borderRadius: 10, background: "#0369a1", color: "#fff", border: "none", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>⬇ Export</button>
          <button onClick={() => setShowInvoices(true)} style={{ padding: "10px 18px", borderRadius: 10, background: "#2b537e", color: "#fff", border: "none", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>📄 Factures</button>
          <button
...
>
  ➕ Séance Sup.
        ...
        {fichesFiltrees.length === 0 ? (
          <div style={{ textAlign: "center", padding: "48px 0", color: "#94a3b8", background: "#fff", borderRadius: 14, fontSize: 14 }}>Aucun candidat trouvé.</div>
        ) : (
                ...
                <tbody>
                  {fichesFiltrees.map((f, ri) => (
                    <React.Fragment key={f.idCandidat}>
                      <tr style={{ background: expanded.has(f.idCandidat) ? "#f0f6ff" : (ri % 2 === 1 ? "#fafafa" : "#fff"), borderBottom: "1px solid #f1f5f9", transition: "background .15s" }}>
                        ...
                        <td style={{ padding: "12px 14px", verticalAlign: "middle" }}>
                          <span style={{ padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700, background: f.statut.bg, color: f.statut.color, whiteSpace: "nowrap" }}>{f.statut.label}</span>
                        </td>
                        ...
                        <td style={{ padding: "12px 14px", verticalAlign: "middle" }}>
                          <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                            {f.versements.length > 0 && (
                              <>
                                <button onClick={() => { const d = genererRecuPDF(f.versements[0], f); d.save(`Recu_${f.nom}_${f.prenom}.pdf`); }} style={btnAct("#f0fdf4", "#166534", "#86efac")} title="Télécharger le reçu PDF">⬇ Reçu</button>
                                <button onClick={() => { const d = genererRecuPDF(f.versements[0], f); window.open(d.output("bloburl"), "_blank"); }} style={btnAct("#f0f4fa", "#2b537e", "#93c5fd")} title="Imprimer le reçu">🖨 Imprimer</button>
                              </>
                            )}
                            <button onClick={() => setHistoriqueCand(f)} style={btnAct("#f5f3ff", "#6d28d9", "#c4b5fd")} title="Historique PDF">📋 Historique</button>
                            <button onClick={() => setRappelCand(f)} style={btnAct("#fff7ed", "#f97316", "#fed7aa")} title="Rappel par email">📧 Rappel</button>
                            {canAddPayment && f.montantRestant > 0 && (
                              <button onClick={() => { setSelected(f); setShowModal(true); }} style={btnAct("#ecfdf5", "#166534", "#6ee7b7")}>+ Versement</button>
                            )}
                          </div>
                        </td>
                      </tr>
        ...
        {historiqueCand && <HistoriqueModal  candidat={historiqueCand} onClose={() => setHistoriqueCand(null)} />}
      </div>
    </div>
  );
};
